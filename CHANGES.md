...
- Fix: Resolved CI/CD E2E test failure in `should observe headings and render TOC chunked progressive list` where `#page-nav-left` overlay element intercepted Playwright clicks; used `{ force: true }` on `#reader-viewport` click to bypass actionability overlay checks and trigger header visibility toggle deterministically (ID: 146).
...
- Documentation: Closed Backlogs 115-118 and Issues 134-137 (ID: 115, 116, 117, 118, 134, 135, 136, 137).
...
- Extended `embed-build-info` Makefile target to append `?v=<BUILD_ID>` cache-buster query parameter to all `<script src="src/js/...">` tags in `index.html`, preventing browsers from serving stale cached JS files after a new build (ID: 060).
- Updated GitHub Pages deploy workflow (`static.yml`) to also apply the cache-buster `?v=` rewriting to `<script>` tags during CI deployment (ID: 060).
