...
3. **Verify 7 Distinct Angles**:
   ...
   - **Security**: Verify input validation, HTML output escaping (to prevent XSS), absence of secrets/credentials, and conformity to secure web development practices. **Cross-reference with [MNG-07](../docs/MNG-07-threat_modeling.md) to ensure all identified STRIDE threats (e.g. Tampering with LocalStorage, Denial of Service via massive files, Elevation of Privilege via XSS) have their defined mitigations correctly implemented.**
   - **Performance**: Look out for memory leaks, redundant computations, heavy DOM manipulations, or slow DB queries.
   - **Design & Requirements Alignment**: Cross-check with high-level and low-level design specifications in `docs/`. **Ensure compliance with [MNG-00](../docs/MNG-00-development_philosophy.md) principles (e.g. strict RTL/LTR reading direction alignment, 1-column layout constraint for mobile viewports, serverless client-side execution, and synchronized manual/keyboard/touch interaction channels).**
