...
6. **抽象構文木（AST）への変換および評価 (`formatAozoraMarkup`)**: 各行に対して字句解析、構文解析、および評価を行い、セキュアな HTML 出力を組み立てます。

#### AST パイプライン仕様
* **字句解析 (Lexer / Tokenizer)**:
  - 行テキストをトークンへ分割します。
  - ルビ（`｜...《...》`、`漢字《...》` などの自動判定および境界特定）、傍点（`［＃傍点］...［＃傍点終わり］`）、太字、斜体、およびプレーンテキスト（`TEXT`）を一連のトークン配列に抽出します。システム側で解釈しない注記（`［＃.+?］`）はこの段階で除去（無視）されます。
  - **ルビの自動判定範囲規則**:
...
* **構文解析 (Parser)**:
  - 抽出されたトークンストリームから木構造の `ASTNode` ツリーを構築します。
...
* **評価器 (Evaluator / Code Generator)**:
  - 構築した AST ノード階層を深さ優先探索（DFS）で巡回し、ホワイトリストに適合する HTML タグを生成します。
  - **エスケープ処理 (XSS対策)**: 評価器が `Text` ノードおよび `Ruby` ノードのテキストを出力する際に、自動的に `escapeHTML(value)` を適用して文字実体参照にエスケープします。エスケープ処理が AST 評価の最下層にカプセル化されているため、不整合なタグ置換や文字の順序依存による HTML インジェクションのリスクを根本から防ぎます。
  - 評価器の出力マッピング：
    - `Root` -> 子ノードの HTML 結合
