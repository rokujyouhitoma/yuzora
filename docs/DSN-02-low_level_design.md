...
- **傍点（強調マーク）注記**:
  `［＃傍点］...［＃傍点終わり］` を検出した際、傍点表示クラスを適用します。
  - 正規表現: `/［＃傍点］(.+?)［＃傍点終わり］/g`
  - 置換後: `<span class="em-sesame">$1</span>`
- **太字ブロック修飾**:
  `［＃ここから太字］...［＃ここで太字終わり］` を `<strong>` 要素へ変換し、CSS で強調表示します。
  - 正規表現: `/［＃ここから太字］(.+?)［＃ここで太字終わり］/g`
  - 置換後: `<strong class="aozora-bold">$1</strong>`
- **斜体ブロック修飾**:
  `［＃ここから斜体］...［＃ここで斜体終わり］` を `<em>` 要素へ変換し、CSS で斜体表示します。
  - 正規表現: `/［＃ここから斜体］(.+?)［＃ここで斜体終わり］/g`
  - 置換後: `<em class="aozora-italic">$1</em>`
- **配置指定（地付き・地寄せ・地から字上げ）**:
  行ごとの位置配置クラスを取得し、段落要素（`<p>`）のクラスに結合します。
  - **地付き**: `［＃地付き］` -> クラス名 `chitsuki`
  - **地寄せ**: `［＃地寄せ］` -> クラス名 `chiyose`
  - **地から○字上げ**: `［＃地から([０-９0-9]+)字上げ］` -> クラス名 `chitage-n` (全角数字は半角に変換)
- **その他システム注記の除去**:
  上記以外の `［＃ここから...］` や `［＃ここで...］` など、システムが解釈しない注記を一括で除去します。
  - 正規表現: `/［＃.+?］/g`
  - 置換後: `""` (空文字)
- **制御文字の除去**:
