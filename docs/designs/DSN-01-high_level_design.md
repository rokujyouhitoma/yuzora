...
graph TD
    subgraph Browser["Webブラウザ (Client-side)"]
        ...
        subgraph Controller["Controller (制御層)"]
            ...
            subgraph JSModules["JS modules/* (ドメイン固有)"]
                ...
                subgraph JS_M_Parser["parser/ (解析・コンパイル)"]
                ...
                subgraph JS_M_Storage["storage/ (データ永続化・アセット)"]
                    JS_Repository["repository.js"]
                    JS_Asset["asset.js"]
                    JS_Resource["resource-director.js"]
                end
                subgraph JS_M_UI["ui/ (表示・UI)"]
                    JS_UI["ui.js"]
    ...
    JSModules <-->|"インポート書籍の保存/復元"| IndexedDB
...
| **作品カードプレースホルダー** | `.book-card-skeleton` (静的/動的) | 作品グリッド内 | ロード完了までの間に表示されるスケルトンスクリーン。実際のカードと同じ比率を維持してシマーアニメーションを適用する。 |
...
開発者向けのアプリ状態モニター、レイアウト/見切れ診断、および `localStorage` 操作を行うモーダル画面です。コンテナ要素は `#debug-modal`（クラス名: `.debug-modal`）で、背面には暗幕 `#debug-modal-overlay`（クラス名: `.debug-modal-overlay`）がオーバーレイされます。モーダルウィンドウは、丸角・外枠や背面暗幕といった質感・外観を美しく維持しつつ、画面内の大部分を有効活用できるよう、幅は最大 1200px（モバイル時は 90% に縮小フィット）、高さは最大 90vh の広さに自動拡張される設計を採用し、操作履歴や診断レポートの一覧性を最大化しています。
...
- 複数セクションによる横並びレイアウトは各セクションの横幅 of 端数計算のズレが累積してレイアウト崩れを引き起こす原因となるため、ドキュメント全体を単一のマルチカラムコンテナ（`.reader-content`）に格納します。改ページ（改段）位置は、コンテナ内に `<div class="page-break"></div>` を挿入し、CSS の `break-before: column` プロパティを適用することで、ブラウザにネイティブで正しいカラム分割を行わせます。これに同期させてカラム幅（`column-width`）をモバイル時は `calc(100% - var(--reader-viewport-padding-x) * 2)`、PC時は見開き2カラム `calc(50% - var(--reader-viewport-padding-x) * 2)`、隙間（`column-gap`）を `calc(var(--reader-viewport-padding-x) * 2)` に設定することにより、各ページは左右に均等な内部安全パディングを確保して美しくセンタリングされ、累積的なレイアウトずれや見切れを完全に排除します。
- **自己修復レイアウトエンジン（動的改ページ挿入による自動補正）**:
  ブラウザの端数計算誤差やフォントのレンダリング誤差によって、まれに通常の段落（`<p>`）内の行や文字が物理的なカラム境界をまたいで半分にスライスされてしまう（見切れる）現象が発生します。これを機械的に解決するため、ロード時、設定変更時、およびリサイズ時に自動レイアウト自己修復エンジンが動作します。このエンジンは、境界線と交差する文字のある段落の直前に動的改ページ要素（`<div class="page-break dynamic-page-break"></div>`）を挿入し、段落全体を次のカラムへと送り出すことで見切れを全自動で回避します。

### 3.1.1 改ページ自動化およびアライメント設計原則 (Page Break Automation & Alignment Principles)
* **静的改ページと動的レイアウト修復の協調**:
  改ページは、ドキュメントの論理的な構造に基づく「静的改ページ（パーサー処理）」と、実行環境の表示寸法や端数誤差を補正する「動的改ページ（レイアウト修復）」の2レイヤーで協調して動作します。
* **見出しの自動改ページと連続時の重複防止**:
  読書の区切りとなる「大見出し」および「中見出し」は、ページの最下部に中途半端に配置されるのを防ぐため、パーサー段階で直前に自動で `PageBreakNode` を挿入します。ただし、大見出しの直後に中見出しが連続する（あるいは空行のみを挟む）場合は、不要な空ページやセクションの意図しない分断を防ぐため、自動改ページの挿入をスキップします。また、小見出しは連続する短いセクションでのページ過多を防ぐため、自動改ページの対象外とします。
* **明示的改ページの確実なアライメント**:
  本文中に明示的に指定された `［＃改ページ］` は、トークン解析を介さずブロックレベルでパースし、余計な段落が生成されるのを防ぐことで、いかなるフォントサイズやビューポート設定であっても確実にカラム境界でアライメントされるようにします。
...
クライアントサイドでのファイル読み込み処理において、悪意のあるスクリプト実行（クロスサイトスクリプティング: XSS）を防止するための対策、および万が一スクリプトが混入した場合の情報漏洩（T-I1）防止対策を行います。
...
- **HTML形式 (.html/.xhtml) の場合**:
  - `DOMParser` を介してドキュメントをパースし、ホワイトリスト方式で安全なHTMLタグ・属性のみを許可するサニタイズ処理（`sanitizeDOM`）を施した上で、コントロールされた特定のコンポーネント配下に安全にレンダリングします。
- **Content Security Policy (CSP) による外部接続・リソース制限**:
  - 万が一悪意あるスクリプトが実行された場合でも、書籍の内容やしおり・設定データが外部サーバーに送信されるリスク（T-I1）を防ぐため、HTMLメタタグによって厳格なCSPを設定します。
  - **ポリシーの構成**:
    - `default-src 'self'`: デフォルトですべてのリソースロードを同一オリジンに限定。
    - `script-src 'self'`: 実行可能なスクリプトを同一オリジン（`src/js/app.js`）のみに限定。インラインスクリプトや `eval`、外部ドメインのスクリプト読み込みを禁止。
    - `style-src 'self' https://fonts.googleapis.com`: 同一オリジンおよび Google Fonts のCSS読み込みを許可。インラインスタイルを制限。
    - `font-src 'self' https://fonts.gstatic.com`: 同一オリジンおよび Google Fonts のフォントファイルの読み込みを許可。
    - `connect-src 'self'`: 外部ドメインへのすべての接続（`fetch`, `XMLHttpRequest` 等）を禁止。ローカルブックのロードなど、同一オリジンへの通信のみを許可。
    - `img-src 'self' data:`: 同一オリジンおよび `data:` スキームのインライン画像の読み込みを許可。
