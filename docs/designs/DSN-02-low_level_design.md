...
* **字句解析器 (`tokenizer.js` / `AozoraTokenizer`)**:
  - `tokenize(text)` メソッド：テキスト全体を入力とし、行分割・空行検出・改ページマーカー検出、および地下げ（字下げ）や揃え、見出し注記などのブロックレベルの抽出とメタデータ解析（字下げ数・アライメント・見出しレベル）を網羅した、ネスト構造のトークン配列を生成して返します（**2段階トークナイズ構造**）。
  - `tokenizeInline(text)` メソッド：各本文ブロックの行テキストをスキャンし、ルビ（`RUBY`）、太字や斜体、傍点（`BOLD_START/END` 等）のインライントークンへと分割します。
...
* **構文解析器 (`parser.js` / `AozoraParser`)**:
  ...
  - `parseTokensToAST(tokens)` メソッド：Tokenizer から引き渡されたインライントークン配列を受け取り、ネスト用スタック（`stack`）を用いて、具象 ASTNode インスタンス（`TextNode`, `RubyNode` 等）を生成・アタッチしてインラインの `ASTNode` ツリーを構築します。
  - **難読化回避のためのブラケット記法**: トークンプロパティへ安全にアクセスできるよう、Parser 内のすべてのトークンオブジェクトの読み出しをブラケット記法（例: `block['type']`）に統一します。
...
- **配置指定（地付き・地寄せ・地から字上げ）**:
  - 行頭または行末に存在する配置指定は、インラインのパースに先立って字句解析器（`AozoraTokenizer.tokenize`）内で検出・抽出・除去され、ブロックトークンのメタデータとして保持されます。その後、パーサー（`AozoraParser`）により、段落要素（`ParagraphNode` 等）のクラスとして統合出力されます。
