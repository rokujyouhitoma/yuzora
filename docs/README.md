...
* **`MNG-xx` (Management)**: 開発プロセス、文書管理、問題管理などの管理・プロセス文書。
* **`REQ-xx` (Requirements)**: 要求定義、機能一覧、システム要件定義などの仕様・制約。
...
* すべての相互参照は、相対パス（例: `[MNG-01](MNG-01-document_ledger.md)` や `[app.js](../src/js/app.js)`) で記述してください。
...
* 競合が発生した場合の分掌ルール（例: 要件定義が基本設計より優先されるなど）は、[[MNG-01] 文書管理・ドキュメント台帳](MNG-01-document_ledger.md) の棲み分け定義に従って一元管理してください。
