...
| **T-S2** | **Spoofing**<br>(なりすまし) | ResourceDirector | 外部から悪意あるアセットパスを注入され、意図しない外部URLからアセットを読み込ませられる。 | Medium | ・`ResourceDirector` でのロード時にオリジン制限およびフェッチ先URLのバリデーションを行い、外部への非許可コネクションを遮断する。 | ・CSP設定（connect-src）と連動したフェッチバリデーションの適用。 | Open | - |
| **T-D2** | **Denial of Service**<br>(サービス拒否) | ResourceDirector, Memory | アセットの解放漏れによるメモリリーク、または巨大なアセットのロードによるメモリ圧迫。 | Medium | ・`ResourceDirector` でアセットの解放処理（`dispose()`によるガベージコレクションの明示的誘導）を一元管理し、ロード時のファイルサイズ制限を強制する。 | ・キャッシュの最大面数制限（LRUキャッシュなど）を導入。 | Open | - |
