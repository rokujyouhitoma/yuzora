...
| **T-T3** | **Tampering**<br>(改ざん)<br>**Elevation of Privilege**<br>(権限昇格) | CommandHistory, importJSON | デバッグインポート機能から悪意ある操作履歴JSONを取り込ませ、パラメータにスクリプトを注入してXSSを引き起こす、または `UpdateConfig` のキーに `__proto__` などのプロトタイプ汚染属性を指定してアプリを乗っ取る。 | High | ・（未実装）`importJSON` 時のスキーマ・パラメータ制限の導入、およびプロトタイプキー（`__proto__`, `constructor`, `prototype`）の完全拒否リスト化。 | ・コマンド履歴デシリアライズ時の Whitelist バリデーションの実装（Issue 043 にて実装予定）。 | Open | [Issue 043](../issues/043-command-history-validation-and-csp.md) |
