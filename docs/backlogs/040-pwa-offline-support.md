...
システム設計討論（ID: 043）におけるネットワークスペシャリスト（NW）の要請に基づき、以下の課題を解決します：
...
- [ ] Chrome DevTools の Application パネルにて `manifest.json` がエラーなく認識され、PWA インストール可能（Installable）と判定されること。
...
- [ ] 新設した E2E テスト (`tests/e2e/pwa.spec.js`) および既存の単体・統合テストが全件パスすること。
